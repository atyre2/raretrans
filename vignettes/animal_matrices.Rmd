...
kite_parms2 <- kite_parms %>% 
  mutate(env_state = rep(env_state[c(1,4,7)], each = 3),
         ...
         plot_s = map2(alpha_s, beta_s, ~tibble(x = seq(0,1,0.01),
                                                d_s = dbeta(x, .x, .y)))) %>% 
  unnest(cols = c("plot_ns", "plot_s"), names_repair = "unique") %>% 
...
exceptions are treated would affect the distribution of uncertainty, and consequently the
