...
```{r}
testdata <- results$observations[[3]]
...
Try a simulation where the initial population are all above ground plants in the stable stage distribution (remove seeds)

```{r}
initial_stages <- c(0, 1, 1, 1, 1, 1, 1, 1)

results <- tibble(observations = map(1:100, 
                                     function(x) sim_observations(TF, N = 100, initial_stages = initial_stages)),
       TF = map(observations, projection_matrix, fertility = "Sd", TF = TRUE),
       N = map(observations, get_state_vector),
       Tunif = map2(TF, N, fill_transitions, P = transition_prior),
       Funif = map2(TF, N, fill_fertility, alpha = fertility_alpha, beta = fertility_beta),
...
In some ways this is worse -- none of the matrices get lambda > 1. It's almost so bad as to be unbelievable. What about a larger sample size, and
only plants > 1 year old?
...
OK, you have to have all stages included in the observation or you can't 
estimate the transitions! Duh ... 
