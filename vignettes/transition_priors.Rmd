...
```{r fillDirichlet, fig.cap = "Asymptotic growth rate using a uniform prior with a total weight of 1 vs. the asymptotic growth rate for the observed data."}
testing <- allA %>% mutate(Agamma = map2(TF, N, fill_fecundity, alpha = c(NA_real_, NA_real_, 0.00001), beta = c(NA_real_, NA_real_, 0.00001)),
...
The matrices with $\lambda = 0$ typically have very few individuals. Matrices with $\lambda = 1$ usually have transitions only on the main diagonal. The uninformative Gamma prior does not change $lambda$, and does not improve the ergodicity and irreducibility issue. 
...
mnCI <- list(ci_95 = MultinomialCI::multinomialCI(c(1, 7, 0, 3), alpha = 0.05),
             ci_90 = MultinomialCI::multinomialCI(c(1, 7, 0, 3), alpha = 0.1),
  ...
  mutate(p = rep(c(1, 7, 0, 3) / 11, times=3),
         ...
         transition =  factor(rep(c("p to p", "p to j", "p to a", "p to m"), times=3), levels = c("p to p", "p to j", "p to a", "p to m"))) %>%
  filter(transition != "p to m")
...
params <- tribble(
  ...
  "RLT, w = 1", "p to a",     0,.01,
  "RLT, w = 1", "p to m",     3,.69,
  "RLT, w = 0.5N", "p to p",     1, 1.375,    
  "RLT, w = 0.5N", "p to j",     7, .275,
  "RLT, w = 0.5N", "p to a",     0, .055,
  "RLT, w = 0.5N", "p to m",     3, 3.795,
  "RLT, w = N", "p to p",     1, 2.75,    
  "RLT, w = N", "p to j",     7, .55,
  "RLT, w = N", "p to a",     0, .11,
  "RLT, w = N", "p to m",     3, 7.59
) 
...
xx <- left_join(xx, params, by = c("prior", "transition")) %>%
  mutate(dens = dbeta(prob, a, b),
         ...
         transition = factor(transition, levels = c("p to p", "p to j", "p to a", "p to m"))) 
...
ggplot(filter(xx, transition != "p to m", prior != "None"), aes(x = prob, y=dens)) + #geom_area(aes(alpha = layer)) +
  geom_area(alpha = 0.25) +
  geom_area(data = function(x)x[x$in95,], alpha = 0.5) + 
  geom_area(data = function(x)x[x$in90,], alpha = 0.75) + 
  geom_area(data = function(x)x[x$in50,], alpha = 1) + 
  geom_line(aes(y=prior_dens)) + 
  theme_classic() +
  geom_point(data = mnCI, mapping = aes(x=p, y=1)) +
  geom_errorbarh(data = mnCI, mapping = aes(x = p, xmin=lcl, xmax=ucl, size=interval_width, y=1), width=0)+
