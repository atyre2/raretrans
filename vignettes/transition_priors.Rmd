---
title: "Effect of prior information on transition probabilities and fertility"
author: "Drew Tyre"
...
A <- projection.matrix(as.data.frame(L_elto), stage="stage", fate="next_stage", fertility="fertility",
                       sort=c("p","j","a"))
knitr::kable(A, digits=2, caption = "Average transition matrix over all census periods and populations.")
```
...
diffPriors <- list()
diffPriors[["Uniform"]] <- testing %>% mutate(Tprior = map2(TF, N, fill_transitions, returnType = "T"),
...
diffPriors[["RLT, weight = 1"]] <- testing %>% mutate(Tprior = map2(TF, N, fill_transitions, 
...
diffPriors[["RLT, weight = 0.5N"]] <- testing %>% mutate(Tprior = map2(TF, N, fill_transitions, 
...
diffPriors[["RLT, weight = N"]] <- testing %>% mutate(Tprior = map2(TF, N, fill_transitions, 
...
TODO: Check that fertility is used somewhere! 
...
mnCI <- list(ci_95 = MultinomialCI::multinomialCI(c(1, 7, 0, 3), alpha = 0.05),
  ...
  mutate(p = rep(c(1, 7, 0, 3) / 11, times=3),
         ...
         transition =  factor(rep(c("s to p", "s to j", "s to a", "s to m"), times=3), levels = c("s to p", "s to j", "s to a", "s to m"))) %>%
  filter(transition != "s to m")

xx <- crossing(prior = c("None", "Uniform", "RLT, w = 1", "RLT, w = 0.5N", "RLT, w = N"),
         transition = c("s to p", "s to j", "s to a", "s to m"),
...
xx <- left_join(xx, params, by = c("prior", "transition")) %>%
  mutate(dens = dbeta(prob, a, b),
         ...
         transition = factor(transition, levels = c("s to p", "s to j", "s to a", "s to m"))) 
...
params$transition <- factor(params$transition, levels = c("s to p", "s to j", "s to a", "s to m"))
...
ggplot(filter(xx, transition != "s to m", prior != "None"), aes(x = prob, y=dens)) + #geom_area(aes(alpha = layer)) +
  ...
  theme_classic() +
  geom_point(data = mnCI, mapping = aes(x=p, y=1)) +
  geom_errorbarh(data = mnCI, mapping = aes(x = p, xmin=lcl, xmax=ucl, size=interval_width, y=1))+
  facet_grid(transition~prior) + #, labeller=label_wrap_gen(width = 15)) + 
...
diffPriors_lci[["RLT, weight = 1"]] <- data_frame(A = sim_transitions(allA$TF[[71]], allA$N[[71]], P=RLT_Tprior, 
                                                                      alpha = c(NA_real_, NA_real_, 0.025),
                                                                      beta = c(NA_real_, NA_real_, 1), samples = samples), 
                                                  lprior = map_dbl(A, lambda))
