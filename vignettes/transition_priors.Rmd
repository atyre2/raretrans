...
xx$prior <- factor(xx$prior, levels = c("None", "Uniform", "RLT, weight = 1", "RLT, weight = 0.5N", "RLT, weight = N"))
xx$transition <- factor(xx$transition, levels = c("p to p", "p to j", "p to a", "p to m"))
fulldist <- filter(xx, transition != "p to m")
ci_95 <- filter(fulldist, cum >0.025, cum < 0.975)
ci_90 <- filter(fulldist, cum >0.05, cum < 0.95)
ci_50 <- filter(fulldist, cum >0.25, cum < 0.75)

# get text values from ci_95
text_labels <- ci_95 %>%
  group_by(transition, prior) %>%
  summarize(upr_ci = max(prob),
            lwr_ci = min(prob),
            med = prob[which.min(abs(cum-0.5))]) %>%
  mutate(label = sprintf("%.2f (%.2f, %.2f)", med, lwr_ci, upr_ci)) %>%
  filter(upr_ci > lwr_ci)

ggplot(fulldist, aes(x = prob, y=dens)) + 
  geom_area(alpha = 0.25) + 
  geom_area(data = ci_95, alpha = 0.25) + 
  geom_area(data = ci_90, alpha = 0.25) + 
  geom_area(data = ci_50, alpha = 0.25) + 
  geom_text(data=text_labels, 
            mapping = aes(label=label), x=0.75, y=12, size=3) +
  facet_grid(prior~transition) + 
  xlab("Transition Probability") + 
  ylab("Probability Density") + 
  theme_bw()
...
<<<<<<< HEAD
Credible interval for lambda figure 
=======
