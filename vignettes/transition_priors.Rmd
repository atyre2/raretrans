...
Here we explore the population consequences of 3 different ways of solving the unobserved transition problem: ignore it, set a prior distribution ignoring constraints, or set a prior accounting for counting constraints. The first solution treats the transition as a fixed zero. This can have severe consequences for matrix structure [@stott2010reducibility]. The second solution uses a prior beta distribution to quantify the uncertainty in the transition probability. The beta prior is easy to parameterize from the actual observations. However, this solution ignores the inherent correlations between observations of different transitions. A seedling that remains a seedling does not become a juvenile. The observed transitions are multinomial rather than binomial events. This can be accommodated by using a Dirichlet prior distribution, albeit at the cost of increased complexity in the calculations. 
