...
gg2 <- ggplot(plotsumm, aes(x=year, y=N)) +
  geom_line()
if(!knitting){
  i <- imgur(token=imgurtkn)
}
ggboth <- gridExtra::grid.arrange(gg1, gg2)
# gg1
# gg2
if(!knitting){
  ggsave(i$file, plot=ggboth, device="png")
  imgur_off(i)
} else{
  ggboth
}
...
```{r fillDirichlet_prior1, fig.cap="Asymptotic growth rates using prior information vs. the raw observations alone. The horizontal red line indicates $\\lambda$ for the RLT prior. Points for all four priors shown in transparent grey."}
diffPriors <- list()
...
ggplot(filter(xx, transition != "p to m"), aes(x = prob, y=dens)) + geom_area() + 
