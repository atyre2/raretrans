...
TODO: Fix shading. 
Add priors for comparison. 
Check that fertility is used somewhere! 


```{r credibleIntervals1, echo=FALSE, fig.width = 7, fig.height = 7, fig.cap="Credible intervals for transition probabilities out of seedling stage in Population 250, year 5. Shaded bands correspond to 50% (darkest), 90%, 95% and 99.9% (lightest) credible intervals. Smaller intervals are not visible in the p->a transitions when the upper boundary of the interval is smaller than 0.01. Labels are median (lower 95% CI, upper 95% CI). Note that where the median and lower interval are equal to 0.01, the actual value is < 0.01."}
# get multinomial confidence interval for Likelihood based interval on NONE
mnCI <- MultinomialCI::multinomialCI(c(1, 7, 0, 3), alpha = 0.05)
colnames(mnCI) <- c("lcl", "ucl")
mnCI <- as_data_frame(mnCI) %>%
  mutate(p = c(1, 7, 0, 3) / 11,
         prior = factor("None", levels = c("None", "Uniform", "RLT, w = 1", "RLT, w = 0.5N", "RLT, w = N")), 
         transition =  factor(c("p to p", "p to j", "p to a", "p to m"), levels = c("p to p", "p to j", "p to a", "p to m"))) %>%
...
params <- tribble(
  ...
  "RLT, w = 1", "p to a",     0.01,
  "RLT, w = 1", "p to m",     3.69,
  "RLT, w = 0.5N", "p to p",     2.375,    
  "RLT, w = 0.5N", "p to j",     7.275,
  "RLT, w = 0.5N", "p to a",     0.055,
  "RLT, w = 0.5N", "p to m",     6.795,
  "RLT, w = N", "p to p",     3.75,    
  "RLT, w = N", "p to j",     7.55,
  "RLT, w = N", "p to a",     0.11,
  "RLT, w = N", "p to m",     10.59
) 
...
xx <- left_join(xx, params, by = c("prior", "transition")) %>%
  mutate(dens = dbeta(prob, a, b),
         #         layer = cut(prob, breaks = c(0.0, 0.025, 0.05, 0.25, 0.75, 0.95, 0.975,1),
         #                     labels =       c("full","95%","90%","50%","u90%","u95%","ufull" )),
         in95 = prob > 0.025 & prob < 0.975,
         in90 = prob > 0.05 & prob < 0.95,
         in50 = prob > 0.25 & prob < 0.75,
...
ggplot(filter(xx, transition != "p to m", prior != "None"), aes(x = prob, y=dens)) + #geom_area(aes(alpha = layer)) +
  ...
  geom_errorbarh(data = mnCI, mapping = aes(x = p, xmin=lcl, xmax=ucl, y=1))+
