...
Tmat <- L_elto %>% 
  ...
  as.matrix()
Tmat <- Tmat[,-4] #throw away the row for transitions to death
...
We must specify the parameters for the fertility prior on the non-reproducing stages as **missing**, else the result will fill in the entire matrix. 

```{r}
fill_fertilityt(TF, N, alpha = c(NA_real_, NA_real_, 1e-05), beta = c(NA_real_, NA_real_, 1e-05))
```
