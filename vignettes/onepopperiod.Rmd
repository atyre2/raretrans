...
This matrix demonstrates the problem with `fill_fecundity()` -- it won't work 
when a stage produces more than one stage. But let's test it out anyway.

```{r}
F_ <- fill_fecundity(Lep_rup, N, 
                     alpha = c(NA_real_, NA_real_, NA_real_, 1, 4),
                     beta = c(NA_real_, NA_real_, NA_real_, 12, 9)
                     )
F_
```

So the 2nd row in *F* is missing.
